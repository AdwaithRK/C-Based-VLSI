...
void Cipher(state_t* state2, uint8_t RoundKey[AES_keyExpSize])
{
  uint8_t round = 0;
//  uint8_t loop, loop2;
...
//
//  for(loop=0;loop<AES_keyExpSize;loop++)
//	  RoundKey[loop] = RoundKey2[loop];
...
//  for(loop=0;loop<4;loop++)
//	  for(loop2=0;loop2<4;loop2++)
//		  (*state2)[loop][loop2] = (*state)[loop][loop2];
